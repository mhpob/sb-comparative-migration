...
import_umces_tag_info <- function(raw_umces_asmfc_tags, raw_umces_boem_tags,
                                  raw_umces_hrf_tags, raw_umces_hrf_ages){
  ...
  hrf_ages <- readxl::read_excel(raw_umces_hrf_ages)
  setDT(hrf_ages)
  
  hrf_ages[, ':='(age = `Consenus ages`,
                 transmitter = paste0('A69-1303-', sonic_no))]
  hrf_ages[, age := fifelse(age == 'no scales', NA, age)]
  hrf_ages[, age := as.numeric(age)]
  
  hrf <- hrf[hrf_ages[, c('age', 'transmitter')], on = 'transmitter']
